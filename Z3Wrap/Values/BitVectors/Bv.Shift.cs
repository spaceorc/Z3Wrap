...
public readonly partial struct Bv<TSize>
{
    ...
    public static Bv<TSize> operator <<(Bv<TSize> left, int shift) => left.Shl(shift);
    ...
    public static Bv<TSize> operator >>(Bv<TSize> left, int shift) => left.Shr(shift);
    ...
    /// <exception cref="ArgumentException">Thrown when shift amount is negative.</exception>
    public Bv<TSize> Shr(int shift, bool signed = false)
    {
        if (shift < 0)
            throw new ArgumentException("Shift amount must be non-negative", nameof(shift));
