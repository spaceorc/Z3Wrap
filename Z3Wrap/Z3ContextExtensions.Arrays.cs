...
public static partial class Z3ContextExtensions
{
    ...
    public static Z3ArrayExpr<TIndex, TValue> Store<TIndex, TValue>(this Z3Context context,
        Z3ArrayExpr<TIndex, TValue> array, TIndex index, TValue value)
        where TIndex : Z3Expr
        where TValue : Z3Expr
    {
    ...
    {
        return typeof(T) == typeof(Z3BoolExpr) ? NativeMethods.Z3MkBoolSort(context.Handle) :
            typeof(T) == typeof(Z3IntExpr) ? NativeMethods.Z3MkIntSort(context.Handle) :
            typeof(T) == typeof(Z3RealExpr) ? NativeMethods.Z3MkRealSort(context.Handle) :
            throw new InvalidOperationException($"Unsupported expression type: {typeof(T)}");
    }
}