...
[TestFixture]
public class Z3ArrayExprTests : IDisposable
{
    ...
    public void ComplexArrayOperation_NestedStoreSelect_Works()
    {
        ...
        // arr[i] := 10, then arr[j] := 20, then select arr[i] and arr[k]
        ...
        // arr2[k] is unconstrained (whatever arr[k] was)
        
        var result = solver.Check();
        Assert.That(result, Is.EqualTo(Z3Status.Satisfiable));
        
        var model = solver.GetModel();
        var val_i = model.GetIntValue(arr2[i]);
        var val_j = model.GetIntValue(arr2[j]);
        Assert.That(val_i, Is.EqualTo(new BigInteger(10)));
        Assert.That(val_j, Is.EqualTo(new BigInteger(20)));
    }
